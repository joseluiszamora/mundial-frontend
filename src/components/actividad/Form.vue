...
export default {
  ...
  methods: {
    submit () {
      ...
      if (!this.$v.$error) {
        // Make post Values
        const actividad = `id_obra=${this.getObraSelected.id_obra}&nombre=${this.nombreDeActividad}&descripcion=${this.descripcionDeObra}&fecha_inicio=${this.fechaDeInicio}&fecha_fin=${this.fechaDeConclusion}`
        // Save Actividad
        this.save(actividad)
      }
    },
    submitEdit () {
      ...
      if (!this.$v.$error) {
        // Make post Values
        const actividad = `id_actividad=${this.actividadSelected.id_actividad}&id_obra=${this.getObraSelected.id_obra}&nombre=${this.nombreDeActividad}&descripcion=${this.descripcionDeObra}&fecha_inicio=${this.fechaDeInicio}&fecha_fin=${this.fechaDeConclusion}`
        // Edit
        this.edit(actividad)
      }
    },
    save (actividad) {
      http.actividadCreate(actividad).then(res => {
        console.log(res)
        this.closeForm()
      }, (error) => {
        this.showError = true
