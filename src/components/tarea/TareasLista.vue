<template>
  <v-card class="py-2 px-4">
    <div>
      <v-btn fab small :class="(getLayout.color || 'primary') + ' accent-4'" right absolute @click="selectors.search = !selectors.search">
        <v-icon dark>search</v-icon>
      </v-btn>
      <v-btn style="margin-right:50px;" fab small right absolute 
        :class="(getLayout.color || 'primary') + ' accent-4'" 
        @click="refreshTareas()">
...
export default {
  ...
  methods: {
    ...
    refreshTareas () {
      http.tareasActividad(this.actividad.id_actividad).then(res => {
        this.items = res.data
      }, (error) => {
        this.showError = true
        console.log(error)
  ...
  watch: {
    getDisplayTareas (newVal, oldVal) {
      if (newVal.listado) {
        this.refreshTareas()
      }
    }
  ...
  computed: {
    ...mapGetters(['getLayout', 'getDisplayTareas'])
  },
  created () {
    this.refreshTareas()
  }
